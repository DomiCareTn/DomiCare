...
const ReceivedRequests = () => {
    ...
    useEffect(async () => {
        ...
        try {
            const posts = await axios.get(
                `http://192.168.11.97:3000/Transactions/servicerequests/${_id}`
            );
            setFeed(posts.data);
        } catch (err) {
            console.log(err);
    ...
    const AcceptRequest = async (e) => {
        try {
            await axios.put(
                `http://192.168.11.97:3000/Transactions/acceptrequest/${e._id}`,
                { e }
            );
        } catch (err) {
            console.log(err);
    ...
    const DeclineRequest = async (e) => {
        try {
            ...
            await axios.put(
                `http://192.168.11.97:3000/Transactions/deleterequest/${e._id}`,
                { e }
            );
        } catch (err) {
            console.log(err);
