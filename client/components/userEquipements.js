...
const userEquipements = ({ navigation }) => {
    ...
    useEffect(() => {
        axios
            .get(
                `http://192.168.119.162:3000/Equipements/${userData.userData._id}`
            )
            .then((res) => {
                setmyData(res.data);
    ...
    const getEquip = () => {
        axios
            .get(
                `http://192.168.119.162:3000/Equipements/equip/${formData._id}`
            )
            .then((res) => {
    ...
    const saveEquipement = () => {
        ...
        axios
            .post(`http://192.168.119.162:3000/Equipements/saveEquip`, {
                formData,
            })
            .then((response) => {
    ...
    const myDelete = () => {
        console.log("ownerId", formData.ownerId);
        axios
            .delete(`http://192.168.119.162:3000/Equipements/${myData.ownerId}`)
            .catch((error) => {
                console.log(error);
            });
    };
    ...
    return (
        <View>
            {clicked === false ? (
                <View>
                    <Button onPress={() => setClicked(true)}>
                        Add Equipement
                    </Button>
                    <View>
                        {myData.map((item, key) => {
                            return (
                                <View key={key}>
                                    <Card>
                                        ...
                                        <Card.Actions>
                                            <Button onPress={myAlert}>
                                                Delete
                                            </Button>
                                            <Button onPress={getEquip}>
                                                Update
                                            </Button>
                                        </Card.Actions>
                                    </Card>
                ...
                <View>
                    ...
                    <TextInput
                        ...
                        onChangeText={(value) =>
                            setData({ ...formData, picture: value })
                        }
                    />
