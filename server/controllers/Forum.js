...
module.exports = {
    ...
    createComment: (req, res) => {
        ...
        QuestAns.create(req.body.obj)
            .then(() => {
                if (req.body.obj.type === "comment") {
                    QuestAns.updateOne(
                        { _id: obj.postId },
                        { $inc: { commentsCount: 1 } }
                    )
                        .then(() => {
                            if(obj.owner._id !== req.params.postOwner){
                                const data = {
                                    sender: obj.owner,
                                    receiver_id: req.params.postOwner,
    ...
    find_All: async (req, res, next) => {
        try {
            const Quests = await QuestAns.find({ type: "Quest" })
                .sort({ createdAt: -1 })
                .exec();
    ...
    find_All_Comments: async (req, res, next) => {
        try {
            const com = await QuestAns.find({ postId: req.params.id })
                .sort({ createdAt: -1 })
                .exec();
    ...
    delete_comment: async (req, res, next) => {
        try {
            const removedComment = await QuestAns.findOneAndUpdate(
                { _id: req.body._id },
